    ...
    <nav className={`fixed top-0 w-full z-[1000] transition-all duration-300 ${isScrolled ? 'bg-white shadow-soft py-2' : 'bg-transparent py-4'}`}>
      <div className="max-w-7xl mx-auto px-6 h-14 flex justify-between items-center">
        ...
        <Link to="/" className="flex items-center">
          ...
          />
          <span className={`ml-3 text-xl font-black italic tracking-tighter ${isScrolled ? 'text-slate-900' : 'text-slate-900'}`}>GuideGo</span>
        </Link>
        ...
        <div className="hidden lg:flex items-center space-x-8">
          ...
          <NavLink to="/ai-chat" label={t('common.chat') || 'Guide AI'} />
          ...
          {user ? (
            <div className="flex items-center space-x-5">
              ...
              <Link to="/profile" className="flex items-center space-x-4 p-1 rounded-full group">
                <div className={`w-10 h-10 rounded-full flex items-center justify-center font-black text-xs uppercase overflow-hidden ring-4 transition-all group-hover:ring-primary-500/30 ${isScrolled ? 'bg-primary-500 text-white ring-slate-100' : 'bg-slate-900 text-white ring-slate-100'}`}>
                ...
                <div className="flex flex-col">
                   ...
                   <p className={`text-sm font-bold leading-none ${isScrolled ? 'text-slate-900' : 'text-slate-900'}`}>{user.name.split(' ')[0]}</p>
                </div>
              </Link>

              <button 
                onClick={handleLogout}
                className="p-3 bg-slate-50 text-slate-400 hover:bg-red-50 hover:text-red-500 rounded-2xl transition-all active:scale-90"
              >
            ...
            <div className="flex items-center space-x-4">
              ...
              <Link to="/register" className="px-8 py-4 bg-slate-900 text-white rounded-2xl text-[10px] font-black uppercase tracking-[0.2em] shadow-xl hover:bg-primary-500 hover:text-slate-900 transition-all active:scale-95">Join Now</Link>
            </div>
          )}
        </div>
        ...
        <div className="flex items-center space-x-3 lg:hidden">
          <button 
            ...
            className={`p-3 rounded-2xl transition-all active:scale-90 shadow-xl ${isScrolled ? 'bg-white/5 text-slate-900 border border-slate-100' : 'bg-slate-900 text-white'}`}
          >
            ...
            >
              <div className="flex items-center justify-between mb-12">
                 <div className="flex items-center space-x-3">
                    ...
                    <span className="text-xl font-black text-slate-900 italic font-serif">GuideGo</span>
                 </div>
