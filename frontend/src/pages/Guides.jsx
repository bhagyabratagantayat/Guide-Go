...
const Guides = () => {
  ...
  return (
    <div className="flex flex-col lg:flex-row h-screen mt-[-64px] overflow-hidden bg-white">
      {/* Sidebar Discovery List */}
      <div className="w-full lg:w-[480px] bg-white border-r border-slate-100 overflow-y-auto custom-scrollbar flex flex-col shadow-inner z-10 pt-16">
        <div className="p-10 sticky top-0 bg-white/80 backdrop-blur-xl z-10 border-b border-slate-50">
          <motion.div 
          ...
          >
            ...
            <h1 className="text-5xl font-black text-slate-900 tracking-tighter italic font-serif">LOCAL GUIDES</h1>
            ...
            <div className="space-y-8 pb-32">
              {guides.map((guide) => (
                <motion.div 
                  ...
                  whileHover={{ y: -5 }}
                  className="group glass-card p-8 rounded-[3rem] border border-slate-50 hover:border-secondary-500/20 transition-all duration-500 cursor-pointer shadow-premium relative overflow-hidden"
                >
                  <div className="flex items-start space-x-6 mb-8 relative z-10">
                    <div className="relative">
                      <div className="w-24 h-24 rounded-[2rem] bg-secondary-50 p-1.5 shadow-inner">
                        <div className="w-full h-full rounded-[1.5rem] bg-white flex items-center justify-center text-secondary-600 font-black text-3xl italic font-serif uppercase overflow-hidden border-2 border-white shadow-xl">
                      ...
                      <div className="absolute -bottom-1 -right-1 w-8 h-8 bg-white rounded-full p-1 shadow-2xl">
                        <div className="w-full h-full bg-green-500 rounded-full animate-pulse ring-4 ring-green-500/20"></div>
                      </div>
                    </div>
                    
                    <div className="flex-1 pt-2">
                      <h4 className="text-2xl font-black text-slate-800 tracking-tighter italic font-serif leading-none group-hover:text-secondary-600 transition-colors uppercase">
                      ...
                      <div className="flex flex-col mt-4 space-y-2">
                        <div className="flex items-center text-xs font-black text-yellow-500 uppercase tracking-[0.2em]">
                          ...
                          {guide.rating} <span className="text-slate-300 mx-2">/</span> <span className="text-slate-500">150+ Tours</span>
                        </div>
                  ...
                  <div className="grid grid-cols-2 gap-4 mb-8 relative z-10">
                    <div className="bg-slate-50 p-4 rounded-[1.75rem] border border-slate-100 flex flex-col justify-center">
                      ...
                      <p className="text-xs font-black text-slate-700 truncate">{guide.languages.slice(0, 2).join(', ')}</p>
                    </div>
                    <div className="bg-slate-900 p-4 rounded-[1.75rem] shadow-xl flex flex-col justify-center">
                      <p className="text-[9px] font-black text-slate-500 uppercase tracking-widest mb-1.5 flex items-center">
                  ...
                  <Link 
                    to={`/guides/${guide._id}`}
                    className="flex items-center justify-between w-full py-5 px-8 bg-secondary-500 text-slate-900 rounded-[2rem] font-black text-[10px] uppercase tracking-[0.3em] hover:bg-slate-900 hover:text-white transition-all shadow-premium active:scale-95 group/link"
                  >
              ...
              {guides.length === 0 && !loading && (
                <div className="text-center py-24 px-12 glass-card rounded-[4rem] border-2 border-dashed border-slate-100 space-y-6">
                  <div className="w-20 h-20 bg-slate-50 rounded-[2.5rem] flex items-center justify-center mx-auto text-slate-200">
                    <User className="w-10 h-10" />
                  </div>
                  <div className="space-y-2">
                    <h3 className="text-xl font-black text-slate-800 tracking-tighter italic">No Experts Found</h3>
                    <p className="text-slate-400 font-bold text-xs uppercase tracking-widest leading-relaxed">Try adjusting your exploration radius.</p>
                  </div>
                </div>
              )}
            </div>
      ...
      <div className="flex-1 relative z-0">
          ...
          {guides.map((guide) => (
            ...
            >
              <Popup className="premium-discovery-popup">
                <div className="p-8 min-w-[280px] bg-white rounded-[3rem] shadow-premium">
                  <div className="flex items-center space-x-4 mb-8 border-b border-slate-50 pb-6">
        ...
        <div className="absolute top-10 right-10 flex flex-col space-y-4">
           <button 
             ...
             className="w-16 h-16 bg-white hover:bg-slate-900 hover:text-white text-slate-900 rounded-[1.75rem] flex items-center justify-center shadow-premium transition-all active:scale-95 group"
           >
              <Compass className="w-7 h-7 group-hover:rotate-45 transition-transform duration-500" />
           </button>
           <button 
             onClick={() => window.location.reload()}
             className="w-16 h-16 bg-white hover:bg-slate-900 hover:text-white text-slate-900 rounded-[1.75rem] flex items-center justify-center shadow-premium transition-all active:scale-95"
           >
