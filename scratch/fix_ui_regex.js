...
// Use regex for loose matching of whitespace and line endings
const otpRegex = /<p className="text-\[10px\] text-slate-500 font-bold mt-8 uppercase tracking-widest italic">Share this with \{matchedGuide\?\.name\} to start trip<\/p>\s+<\/div>/;
...
if (otpRegex.test(content)) {
    content = content.replace(otpRegex, otpReplacement);
    console.log('OTP section updated');
}

const ongoingRegex = /<div className="bg-\[#1e293b\] p-10 rounded-\[3rem\] flex items-center gap-8 border border-white\/5">[\s\r\n]*<img src=\{matchedGuide\?\.profilePicture \|\| 'https:\/\/i\.pravatar\.cc\/150'\} className="w-20 h-20 rounded-\[1\.8rem\] object-cover" alt=""\/>[\s\r\n]*<div><h4 className="font-black text-2xl text-white italic">\{matchedGuide\?\.name\}<\/h4><p className="text-\[11px\] font-bold text-slate-500 flex items-center gap-2 mt-2 uppercase tracking-widest"><Navigation size=\{14\} className="text-blue-500" \/> Active Trip<\/p><\/div>[\s\r\n]*<\/div>/;
...
if (ongoingRegex.test(content)) {
    content = content.replace(ongoingRegex, ongoingReplacement);
